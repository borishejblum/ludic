...
match_2in1_2datasets <- function(data1, data2, ind_match, 
  ...
  id_falsematch <- as.character(ind_falsematch)
  ...
  tmp_Genom <- c(data1_out[id_match, "Genom"], 
                 rbinom(n2-nb_match-nb_falsematch, size=2, prob=MAF0),
                 data1_out[id_falsematch, "Genom"])
  tmp_Pheno <- c(data1_out[id_match, "Pheno"], 
                 rbinom(n2-nb_match-nb_falsematch, size=1, prob=expit(-0.5+log(OR)*tmp_Genom[-c(1:nb_match, (n2-nb_falsematch):n2)])),
                 data1_out[id_falsematch, "Pheno"])
  ...
  sstdFit_1way <-  try(sstdFit(x=sample(dist_all,10000)))
  if(inherits(sstdFit_1way, "try-error")){sstdFit_1way <-  try(sstdFit(x=sample(dist_all,10000)))}
  tmp_est <-  sstdFit_1way$estimate
  for(i in 1:4){
    sstdFit_1way_sub <-  try(sstdFit(x=sample(dist_all,10000)))
    if(inherits(sstdFit_1way_sub, "try-error")){sstdFit_1way_sub <-  try(sstdFit(x=sample(dist_all,10000)))}
  ...
  rank_match_2 <- sapply(colnames(dist_all), FUN=matchProbs_rank_complete, computed_dist=t(dist_all),
                         prop_match=prop_match_1way)
  ...
  ## combine optimally two thresholds with two influence functions ####
  ...
  for(cut_p in thresholds){
    ...
    eta[[as.character(cut_p)]] <- rbind((1*xi), (1*xi))*inv_fishInfo_mat%*%t(Z)%*%diag(x=(mydata_match[,"Pheno"] - expit(Z%*%theta)[,1]))
  }
  eta_mat <- sapply(X=eta, FUN=function(m){m[2,]})
  selected_thres <- c(3,9)#c(0:4)*2+1##
  sigma <- crossprod(eta_mat[, selected_thres])
  #cor_mat <- diag(1/(sqrt(diag(sigma))))%*%sigma%*%diag(1/(sqrt(diag(sigma))))
  sigma_inv <- solve(sigma)
  one <- rep(1, nrow(sigma_inv))
  ...
  weighted_sigma <- sigma*tcrossprod(weights)
  se_delta <- sqrt(sum(weighted_sigma))
  ...
  est_threshold_pred <- predict.lm(lmfit_threshold, se.fit = T, newdata=cbind.data.frame("x"=1))
  ...
  B <- 5000
  est_threshold_pred_pb <- numeric(B)
  for(i in 1:B){
    #     y.pb<-NULL
    #     for(j in 1:nrow(lmfit_threshold_data)){
    #       y.pb <- c(y.pb, rnorm(1, mean=lmfit_threshold_data$y[j], sd=lmfit_threshold_data$y.se[j]))
    #     }
    #     data_pb <- cbind.data.frame(y.pb, "x"=lmfit_threshold_data$x)
    data_pb <- cbind.data.frame("y.pb"=rmvn(1, mu=lmfit_threshold_data$y, sigma=sigma)[1,],#rnorm(n=nrow(lmfit_threshold_data), mean=lmfit_threshold_data$y, sd=lmfit_threshold_data$y.se),
                                "x"=lmfit_threshold_data$x
    )
    lmfit_threshold_pb <- lm(y.pb~x, data=data_pb)
    est_threshold_pred_pb[i] <- predict.lm(lmfit_threshold_pb, se.fit = T, newdata=cbind.data.frame("x"=1))$fit
  }
  ...
  result <- c(nmatch_hat_1way, accuracy_all, betahat_all, 
              mean(est_threshold_pred_pb), sd(est_threshold_pred_pb), 
              theta_delta, se_delta, 
              nmatch_final_all, epsilon_truth, n_freq1, n_freq5, n_freq10)
  write(result, file=paste0(outputfile_name,"_noagg.txt"), append=T, ncol=500)
}