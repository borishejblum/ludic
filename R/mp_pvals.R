#'P-values from matched probabilities
#'
...
mp_pvals <- function(data1, data2, ind_match, 
  ...
  id_nomatch1 <- ind1[!(ind1 %in% id_match)]
  id_nomatch2 <- ind2[!(ind2 %in% id_match)]
  ...
  # data Desc ####
  #codenum overall
  codenum1 <- rowSums(data1_bin)
  codenum2 <- rowSums(data2_bin)
  avg_codenum1 <- mean(codenum1)
  avg_codenum2 <- mean(codenum2)
  avg_codenum <- mean(c(codenum1, codenum2))
  #codenum matches
  avg_codenum_matches1 <- mean(codenum1[id_match])
  avg_codenum_matches2 <- mean(codenum2[id_match])
  avg_codenum_matches <- mean(c(codenum1[id_match], codenum2[id_match]))
  #codenum nonmatches
  avg_codenum_nomatches1 <- mean(codenum1[id_nomatch1])
  avg_codenum_nomatches2 <- mean(codenum2[id_nomatch2])
  avg_codenum_nomatches <- mean(c(codenum1[id_nomatch1], codenum2[id_nomatch2]))
  data1_bin_desc <- data1_bin
  data2_bin_desc <- data2_bin
  
  
  #commoncodenum overall
  avg_commoncodenum <- common1Mean_C(data1_bin, data2_bin)
  #commoncodenum matches
  avg_commoncodenum_matches_noperturb <- mean(rowSums(data1_bin[id_match, ] * data1_bin[id_match, ]))
  avg_commoncodenum_matches <- mean(rowSums(data1_bin[id_match, ] * data2_bin[id_match, ]))
  #commoncodenum nomatches
  avg_commoncodenum_nomatches <- common1Mean_C(data1_bin[id_nomatch1, ], data2_bin[id_nomatch2, ])
  ...
  epsilon_kplus_truth <- sapply(1:ncol(data2_bin),function(v){
    ...
    #mean(data2_bin[id_match[data1_bin[id_match,v]==0],v]==1)
  })
  ...
  for(cut_p in thresholds){
    ...
    falsematch <- matchClaimed_mat_indexes[which(rownames(probMax_mat)[matchClaimed_mat_indexes[,1]] != colnames(probMax_mat)[matchClaimed_mat_indexes[,2]]),]
    mismatch_avg[as.character(cut_p)] <- mean(rowSums(data1_bin_desc[falsematch[,1],]*data2_bin_desc[falsematch[,2],]))
  }
  ...
  FSem_res <- em_winkler(data1_bin, data2_bin)
  score_threshold_em <- FSem_res$threshold_ms
  
  matchClaimed_mat_em <- (FSem_res$matchingScore >= FSem_res$threshold_ms)
  matchClaimed_mat_indexes_em <- which(matchClaimed_mat_em, arr.ind = TRUE)
  nmatch.hat_final_em <-  FSem_res$n_truematches
  if(is.null(rownames(FSem_res$matchingScore))){
    rownames(FSem_res$matchingScore) <- 1:nrow(FSem_res$matchingScore)
  }  
  if(is.null(colnames(FSem_res))){
    colnames(FSem_res$matchingScore) <- 1:ncol(FSem_res$matchingScore)
  }
  TPR_em <- min(1, sum(rownames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em[,1]] == colnames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em[,2]])/length(id_match))
  FPR_em <- sum(rownames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em[,1]] != colnames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em[,2]])/(nrow(data1_bin)*nrow(data2_bin)-length(id_match))
  PPV_em <- sum(rownames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em[,1]] == colnames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em[,2]])/nrow(matchClaimed_mat_indexes_em)
  NPV_em <- (nrow(data1_bin)*nrow(data2_bin)-nrow(matchClaimed_mat_indexes_em)-(length(id_match)-sum(rownames(probMax_mat)[matchClaimed_mat_indexes_em[,1]] == colnames(probMax_mat)[matchClaimed_mat_indexes_em[,2]])))/(nrow(data1_bin)*nrow(data2_bin)-nrow(matchClaimed_mat_indexes_em))
  accuracy_hat_em  <- c("TPR_em"=TPR_em, "FPR_em"=FPR_em, "PPV_em"=PPV_em, "NPV_em"=NPV_em)
  
  
  # EM Winkler 1-1 modif ----
  col_max <- apply(FSem_res$matchingScore, 2, max)
  col_max_mat <- matrix(col_max, nrow=nrow(FSem_res$matchingScore), ncol=length(col_max), byrow = TRUE)
  rm(col_max)
  FSem_res$matchingScore <- FSem_res$matchingScore*(FSem_res$matchingScore>=col_max_mat) + (FSem_res$threshold_ms-1)*(FSem_res$matchingScore<col_max_mat) 
  rm(col_max_mat)
  
  matchClaimed_mat_em_oneone <- (FSem_res$matchingScore >= FSem_res$threshold_ms)
  matchClaimed_mat_indexes_em_oneone <- which(matchClaimed_mat_em_oneone, arr.ind = TRUE)
  nmatch.hat_final_em_oneone <-  sum(matchClaimed_mat_em_oneone)
  if(is.null(rownames(FSem_res$matchingScore))){
    rownames(FSem_res$matchingScore) <- 1:nrow(FSem_res$matchingScore)
  }  
  if(is.null(colnames(FSem_res))){
  ...
  TPR_em_oneone <- min(1, sum(rownames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em_oneone[,1]] == colnames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em_oneone[,2]])/length(id_match))
  FPR_em_oneone <- sum(rownames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em_oneone[,1]] != colnames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em_oneone[,2]])/(nrow(data1_bin)*nrow(data2_bin)-length(id_match))
  PPV_em_oneone <- sum(rownames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em_oneone[,1]] == colnames(FSem_res$matchingScore)[matchClaimed_mat_indexes_em_oneone[,2]])/nrow(matchClaimed_mat_indexes_em_oneone)
  NPV_em_oneone <- (nrow(data1_bin)*nrow(data2_bin)-nrow(matchClaimed_mat_indexes_em_oneone)-(length(id_match)-sum(rownames(probMax_mat)[matchClaimed_mat_indexes_em_oneone[,1]] == colnames(probMax_mat)[matchClaimed_mat_indexes_em_oneone[,2]])))/(nrow(data1_bin)*nrow(data2_bin)-nrow(matchClaimed_mat_indexes_em_oneone))
  accuracy_hat_em_oneone  <- c("TPR_em"=TPR_em_oneone, "FPR_em"=FPR_em_oneone, "PPV_em"=PPV_em_oneone, "NPV_em"=NPV_em_oneone)
  ...
  result <- c(nmatch_hat_1way, accuracy_all, betahat_all, 
              ...
              nmatch.hat_final_em, accuracy_hat_em, nmatch.hat_final_em_oneone, accuracy_hat_em_oneone, score_threshold_em, 
              avg_codenum1, avg_codenum2, avg_codenum, 
              avg_codenum_matches1, avg_codenum_matches2, avg_codenum_matches,
              avg_codenum_nomatches1, avg_codenum_nomatches2, avg_codenum_nomatches,
              avg_commoncodenum, avg_commoncodenum_matches, avg_commoncodenum_nomatches, avg_commoncodenum_matches_noperturb, mismatch_avg)
